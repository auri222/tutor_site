      ...
      try {
        if (id === "" || id === undefined) {
          ...
          if (res.data.success) {
            ...
            Swal.fire(
              "Chúng tôi đã gửi mã OTP vào email mà bạn cung cấp. Hãy nhập mã vào ô để xác nhận."
            );
          }
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `http://localhost:8000/api/auth/otp/${id}`,
        data
      );
      if (res.data.success) {
        setLoading(false);
    ...
    <div className="otp">
      ...
      <div className="otpContainer">
        ...
        <form className="oForm">
          ...
          <button className="fButton" disabled={!isUser} onClick={handleSubmit}>
            Xác nhận
          </button>
        </form>
      </div>
